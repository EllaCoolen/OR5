...
# def controleer_lege_cellen(planning):
#     """
...
#         lege_cellen = planning[planning[kolom].isnull()]
...
def controleer_koppels(df_koppels, planning):
    ...
    for index, rij in df_koppels.iterrows():
        ...
        adres_a_voor = planning.loc[planning['Bewoner'] == persoon_a, 'Voor'].values
        adres_b_voor = planning.loc[planning['Bewoner'] == persoon_b, 'Voor'].values

        adres_a_hoofd = planning.loc[planning['Bewoner'] == persoon_a, 'Hoofd'].values
        adres_b_hoofd = planning.loc[planning['Bewoner'] == persoon_b, 'Hoofd'].values

        adres_a_na = planning.loc[planning['Bewoner'] == persoon_a, 'Na'].values        
        adres_b_na = planning.loc[planning['Bewoner'] == persoon_b, 'Na'].values

        if adres_a_voor == adres_b_voor and adres_a_hoofd == adres_b_hoofd and adres_a_na == adres_b_na:
            errors += 0
        else:
            errors += 1
    return errors
